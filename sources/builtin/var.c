...
/*   Updated: 2024/02/22 13:59:40 by asacchin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	var_update(char *src, t_list *list, int n)
{
	...
	while (clone)
	{
		if (!ft_strncmp(src, clone->content, var_name_length(src)))
		{
			...
			{
				free(clone->content);
				clone->content = var_no_content_quote(src);
				return (1);
			}
			clone2 = ft_strjoin(clone->content,
				src + var_name_length(src) + 2);
			free(clone->content);
			clone->content = var_no_content_quote(clone2);
			free(clone2);
			return (1);
		}
		clone = clone->next;
...
int	var_new(char *src, t_attr *strc, int tot, int clear)
{
	...
	else if (tot && !clear)
	{
		...
		free(src);
		src = var_no_content_quote(c);
		free(c);
	}
	clone = strc->local_list;
	clone = ft_lstnew(var_no_content_quote(src));
	if (!clone)
