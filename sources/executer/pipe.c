...
/*   Updated: 2023/11/24 16:25:38 by pfalasch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void write_to_pipe(t_attr *att)
{
	...
	if (dup2(att->pipesfd[att->pipe_index][WRITE_END], STDOUT_FILENO) == -1)
...
{
	close(att->pipesfd[att->pipe_index][1]);
	if (dup2(att->pipesfd[att->pipe_index][READ_END], STDIN_FILENO) == -1)
		close(att->pipesfd[att->pipe_index][0]);
}
